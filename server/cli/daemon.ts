 ...
 * (`bfe [path]`), where each process auto-assigns its own free port; the daemon
 ...
 * distinct ports without a reload per collision.
 */
...
export function serverLaunchEnvironment(options: {
  ...
  return {
    ...
    EXPLORER_ROOT: options.root,
    EXPLORER_CONFINE: options.confine ? 'true' : 'false',
    EXPLORER_PORT_STRATEGY: options.explicitPort === undefined ? 'auto' : 'strict',
    EXPLORER_READY_FILE: options.readyFilePath,
  }
}
