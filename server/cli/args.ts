...
export type LaunchOptions = {
  /** Positional arguments after the command — the first is the served path. */
  positionals: string[]
  ...
  open: boolean
  /** Whether the served root is a boundary (true) or only an anchor (false). */
  confine: boolean
}
...
function parseLaunchOptions(
  ...
  let open = true
  let confine = environment.confine
  ...
  for (let index = 0; index < argumentList.length; index++) {
    ...
    if (argument === '--port' || argument === '-p') {
      ...
      open = false
    } else if (argument === '--confine') {
      confine = true
    } else if (argument === '--no-confine') {
      confine = false
  ...
  return { ok: true, options: { positionals, explicitPort, host, open, confine } }
}
