...
const ENVIRONMENT = { host: '127.0.0.1', confine: false }
...
describe('parseCli — command routing', () => {
  ...
  test('a bare path is a serve of that path', () => {
    const intent = intentOf(['/etc'])
    expect(intent.kind).toBe('serve')
    if (intent.kind === 'serve') expect(intent.options.positionals).toEqual(['/etc'])
  })
...
describe('parseCli — flags', () => {
  ...
  test('confine follows the environment default and --confine / --no-confine', () => {
    const off = intentOf(['serve'])
    const on = intentOf(['serve', '--confine'])
    const forcedOff = intentOf(['serve', '--no-confine'], { host: '127.0.0.1', confine: true })
    if (off.kind !== 'serve' || on.kind !== 'serve' || forcedOff.kind !== 'serve') {
      throw new Error('expected serve')
    }
    expect(off.options.confine).toBe(false)
    expect(on.options.confine).toBe(true)
    expect(forcedOff.options.confine).toBe(false)
  })
