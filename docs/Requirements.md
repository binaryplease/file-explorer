...
| [014-relative-links-in-rendered-markdown.md](requirements/014-relative-links-in-rendered-markdown.md) | A relative link in a rendered markdown file leaves the app instead of opening the file it names — `[LICENSE](LICENSE)` in README.md must select that entry, not navigate the browser away. | planned | 14 |
