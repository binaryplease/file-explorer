...
| [012-publication-readiness.md](requirements/012-publication-readiness.md) | Publication is blocked on git history and on the remote's object store — not the tip: commits carry a sibling service's name (one in a subject line), a deleted file maps an outside decision corpus, and an unreferenced commit is still served by SHA after a reset. The route is settled: a new repository, never a rewrite. The tip is now clean and the finish line is a public repository only; what remains is the publication surface and the export itself. | in-progress | 12 |
...
_18 entries — one row per file in `requirements/`._
<!-- index:end -->
